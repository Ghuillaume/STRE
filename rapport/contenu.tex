...
\chapter{Simulateur d'ordonnanceur}
	...
	\section{Conception fonctionnelle}
		...
		\subsection{choix de représentation}
			Nous avons choisi de représenter un temps creux par l'indice -1
...
Finsi
		...
		\subsection{Simulation d'ordonnancement}
			...
			\subsubsection{poll\_needed}
				...
				\paragraph{Post-condition :retourne un booléan qui informe du besoin d'élire un nouvelle tâche à exécuter} 
	...
	//Ordonnanceur appele que si reveil ou terminaison de tache
		...
		retourne need_to_poll
			...
			\subsubsection{init\_context}
				\paragraph{Pré-condition : un algorithme d'ordonnancement a été lancé} 
				\paragraph{Post-condition : le contexte d'exécution des tâches a été initialisé} 
				\paragraph{Objectif : initialiser le contexte d'exécution des tâches} 
				\paragraph{Algo :} 
					\begin{lstlisting}
...
//Initialisation du context : 
 ...
 Finsi
			...
			\subsubsection{MAJ\_context}
				\paragraph{Pré-condition : le contexte a été initialisé} 
				\paragraph{Post-condition : le contexte a été mis à jour} 
				\paragraph{Objectif :mise à jour du contexte d'exécution} 
				\paragraph{Algo :} 
					\begin{lstlisting}
...
Finsi
			...
			\subsubsection{Rate Monotonic + Serveur}
				\paragraph{Pré-condition : le fichier d'entrée a été parsé} 
				\paragraph{Post-condition : les tâches ont été ordonnancées} 
				\paragraph{Objectif : Simuler l'ordonnancement selon RM d'un jeu de tâches} 
				\paragraph{Algo :} 
					\begin{lstlisting}
Entree : 
Sortie :  
...
\chapter*{Conclusion}
%    Ce projet aura été l'occasion de mettre en application les savoirs acquis durant ce module (Fonctionnement des algorithmes, conditions d'ordonnançabilité, ...). De plus, cela nous a permis de réfléchir à des problématiques inhabituelles comme la génération pseudo-aléatoire de fractions ou plus simplement la gestion des tâches par l'ordonnanceur.
%    En plus de tout ça, le projet nous a permis de nous exercer à écrire une spécification logicielle et à l'implémenter, à utiliser git et à utiliser le langage de programmation C++. 
